...
void WorldSession::HandleMoveWorldportAckOpcode()
{
    ...
    // Capture the player guid rather than the session pointer: when the
    // destination transport is not yet on the target map this lambda is
    // deferred through map->GetMessager() and runs on the map worker thread a
    // tick later. The session can be destroyed in that window (e.g. a bot
    // logging out mid-teleport), so a captured raw `this` is a use-after-free
    // (SIGSEGV 2026-08-09 in Map::Update -> this lambda, core.20260809-135821).
    auto lambda = [playerGuid = _player->GetObjectGuid(), loc, old_loc, mEntry, mInstance](Map* map)
    {
        Player* player = sObjectAccessor.FindPlayer(playerGuid, false);
        ...
        session->GetAnticheat()->Teleport({ loc.coord_x, loc.coord_y, loc.coord_z, loc.orientation });
