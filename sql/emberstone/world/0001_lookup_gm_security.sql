...
--
-- Kept as SQL rather than a C++ change to command-table registration
-- because this is a per-server policy choice: the `command` table is
-- designed precisely to override registration defaults per-deployment.
UPDATE `command`
   SET `security` = 2
 WHERE `name` IN (
   'lookup creature',
